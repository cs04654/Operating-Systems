...
static ssize_t copy_to_userspace(struct file *filp, char __user *buff, size_t len, loff_t *loff)
{
	char bffrtst[] = "this is a test :(";
	if(copy_to_user(buff, bffrtst, len))
	{
		printk(KERN_INFO "Read Failed for buffer: %s", buff);
		return -EFAULT;
	}
}*/
